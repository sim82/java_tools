...
public class SimplePing3 {
	...
	class Guard implements Runnable {
		...
		while (!Thread.interrupted()) {
			...
			if (addr != null) {
			    DatagramPacket txp = new DatagramPacket(txb,
				    txb.length / 2, addr, port);
			    try {
				sock.send(txp);
			    } catch (IOException e) {
				// TODO Auto-generated catch block
	...
	while (true) {
		...
		for (int i = 0; i < 100000; i++) {
		    txb[0] = (byte) i;
		    DatagramPacket txp = new DatagramPacket(txb, txb.length,
			    rxp.getAddress(), 12340);
		    sock.send(txp);
		}
		DatagramPacket txp = new DatagramPacket(txb, txb.length / 2,
			rxp.getAddress(), 12340);
		sock.send(txp);
		guard.ping();
