...
public class GigaPing {
	...
	class MyReader implements Runnable {
		...
		while (true) {
			...
			// System.out.printf(
			// "received packet of length: %d from %s\n",
			// rxp.getLength(), rxp.getAddress().toString() );
			byte[] bb = rxp.getData();

			// System.out.printf(
			// "%d received packet of length: %d from %s:%d (%d) (%d %d %d %d)\n",
			// i, rxp.getLength(), rxp.getAddress().toString(),
			// rxp.getPort(), serial, bb[0], bb[1], bb[2], bb[3] );

			DatagramPacket retp = new DatagramPacket(abuf, 4,
				rxp.getAddress(), 21844);
			rxsock.send(retp);
		    } catch (IOException e) {
	...
	DatagramPacket p = new DatagramPacket(buf, buf.length,
		InetAddress.getByAddress(addr), 21845);
