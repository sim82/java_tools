...
public class CheckBinning {
	...
	for (ClassifierOutput.Res res : cf.reslist) {
	    ...
	    LN rtp = LN
		    .getNonTipNode(LN.removeTaxon(LN.deepClone(rt), res.seq)[0]);
		...
		if (cbr.equals(brOrig) || cbr.equals(res.branch)) {
		    // LN[] brOrigRt =
		    // LN.findCorrespondingBranch(LN.findBranchByName(t,
		    // brOrig), rtp );
		    // LN[] brRt =
		    // LN.findCorrespondingBranch(LN.findBranchByName(t,
		    // res.branch), rtp );
		    ...
		    // res.branch, brOrigRt[0].backSupport, brRt[0].backSupport
		    // );
		    continue;
		}
		...
		Set<String> nl1 = ClassifierTwoClades.getBranchLabelSet(LN
			.getAsList(branch[0], false));
		Set<String> nl2 = ClassifierTwoClades.getBranchLabelSet(LN
			.getAsList(branch[1], false));
		...
		if (nl1.contains(brOrig)) {
		    ...
		    throw new RuntimeException(
			    "none of the split trees contains the original branch");
		}
