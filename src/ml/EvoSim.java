...
public class EvoSim {
	...
	for (LN n : list) {
	    if (n.data.isTip && n.back != null) {
		System.out.printf("%s%s\n", MultipleAlignment.padSpaceRight(
			n.data.getTipName(), maxNameLen + 1),
			toString(n.data.seq));
	    }
	}
    }
	...
	for (byte c : seq) {
		...
		throw new RuntimeException(
			"fuckage in generated seq: illegal byte: " + (int) c);
	    }
	...
	default:
	    throw new RuntimeException("unknown character in sequence: "
		    + (char) b);
