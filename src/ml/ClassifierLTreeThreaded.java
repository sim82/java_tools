...
public class ClassifierLTreeThreaded {
	...
	if (!args[0].equals("--auto")) {
	    ...
	    File oltFile = new File(dir, "RAxML_originalLabelledTree."
		    + args[2]);
	    File reftreeFile = new File(args[3]);

	    ClassifierLTreeThreaded clt = new ClassifierLTreeThreaded(
		    reftreeFile, rnFile);
	    ...
	    final ClassifierLTreeThreaded clt = new ClassifierLTreeThreaded(
		    reftreeFile, rnFile);
	    ...
	    int N_THREADS = 1;
	    ThreadPoolExecutor tpe = new ThreadPoolExecutor(N_THREADS,
		    N_THREADS, 0L, TimeUnit.SECONDS,
		    new ArrayBlockingQueue<Runnable>(N_THREADS * 3));
		...
		tpe.execute(new Runnable() {
			...
			{
			    oltFile = new File(classFile.getParent(),
				    "RAxML_originalLabelledTree." + suffix);
				...
				throw new RuntimeException(
					"cannot find olt file (with and without .gz suffix)");
			    }
			}
			...
			try {
			    ...
			    e.printStackTrace();
			    throw new RuntimeException("bailing out at: "
				    + classFile.getName() + " "
				    + oltFile.getName());
			}
		    }
	...
	// list2 ), LN.cmpLNList( list1, list3 ), LN.cmpLNList( list2, list3 ),
	// LN.cmpLNListObjectIdentity( list1, list2 ),
	// LN.cmpLNListObjectIdentity( list1, list3 ),
	// LN.cmpLNListObjectIdentity( list2, list3 ));
	// System.out.printf( "sym: %s %s\n", LN.checkLinkSymmetry(n),
	// LN.checkLinkSymmetry(nn) );
	// }
	...
	try {
		...
		try {
			...
			seqSubIdx = Integer.parseInt(seqOrig
				.substring(atIdx + 1));
		    }
		    ...
		    // name of the branch, the classifier has put the taxon in
		    ...
		    // get the weighted path length between current and original
		    // insertion position
			...
			// - remove the current taxon from the reference tree
			// (copy), so that its topology
			// resembles the pruned tree from the current
			// classification
			// - return the branch from the reference tree that
			// corresponds to the original taxon position
			// LN[] opb = LN.removeTaxon(reftreePruned, seq, false);
			// reftreePruned = opb[0];

			// identify the current insertion position from the
			// pruned tree in the
			// reference tree. The position is identified by the
			// split set (or how ever this thin is called)
			// long time1 = System.currentTimeMillis();
			// the LN referenced by reftreePruned can (!?) never be
			// removed by removeTaxon so it's ok to use it as pseudo
			// root
			...
			// LN[] ipb = LN.findBranchBySplit(reftreePruned,
			// insertSplit);
			LN[] ipb = prt.getTSC().getBranchBySplit(insertSplit);
			// System.out.printf( "time: %d\n",
			// System.currentTimeMillis() - time1 );

			// calculate weighted path length between original and
			// current insertion branches
			// the lengths of the current and original branches
			// contribute half of their weight.
			// if the branches are identical the path has zero
			// length

			int[] fuck = new int[1]; // some things (like 'multiple
						 // return values') are soo
						 // painful in java ...
			lenOT = getPathLenBranchToBranch(prt.getOBP(), ipb,
				0.5, fuck);
			ndOT = fuck[0];
			// {
			...
			//
			// lenOT = Math.min(lenOT1alt, lenOT2alt);// +
			// opb[0].backLen;
			//
			// }
			...
			if (smap != null) {
			    ...
			    out.printf("%s\t%s\t%s\t%d\t%d\t%f\t%f\t%f\t%d\n",
				    seqOrig, branch, realBranch[0].backLabel,
				    support, ndOT, lenOT, lenOT
					    / reftreeDiameter,
				    reftreeDiameter/* , oltDiameter */,
				    seqSubIdx);
			} else {
			    out.printf("%s\t%s\t%s\t%d\t%d\t%f\t%f\t%f\n",
				    seqOrig, branch, realBranch[0].backLabel,
				    support, ndOT, lenOT, lenOT
					    / reftreeDiameter, reftreeDiameter/*
									       * ,
									       * oltDiameter
									       */);
			...
			// for comparison: get the path length in the (possibly
			// unweighted) pruned tree
			// get path len between real position and current
			// insertion position

			double len = getPathLenToNamedBranch(realBranch[0],
				branch, false);
			if (len < 0) {
			    len = getPathLenToNamedBranch(realBranch[1],
				    branch, false);
			}
			// len += realBranch[0].backLen;

			int lenUW = getUnweightedPathLenToNamedBranch(
				realBranch[0], branch, false);
			if (lenUW == Integer.MAX_VALUE) {
			    lenUW = getUnweightedPathLenToNamedBranch(
				    realBranch[1], branch, false);
			}

			// System.out.printf(
			// "%s %s %s %d %d %f %f %f (%f %f)\n", seq, branch,
			// realBranch[0].backLabel, support, lenUW, len, lenOT,
			// lenOT / reftreeDiameter, reftreeDiameter, oltDiameter
			// );
			out.printf("%s\t%s\t%s\t%d\t%d\t%f\t%f\t%f\t%f\n", seq,
				branch, realBranch[0].backLabel, support,
				lenUW, len, lenOT, lenOT / reftreeDiameter,
				reftreeDiameter/* , oltDiameter */);
		    }

		} catch (NoSuchElementException x) {
		    System.out.printf("bad line in raxml classifier output: "
			    + line);
		    x.printStackTrace();
	    ...
	    Logger.getLogger(ClassifierLTreeThreaded.class.getName()).log(
    ...
    static double getPathLenBranchToBranch(LN n[], LN[] b, double seScale,
	    int[] nodedist) {
	...
	if (back) {
	    throw new RuntimeException(
		    "the 'back' flag is not supported for this opperation.bailing out.");
	}
	...
	if (node.next.next.back != null) {
	    double len = getPathLenToNamedBranch(node.next.next.back, name,
		    false);
    ...
    public static int getUnweightedPathLenToNamedBranch(LN node, String name,
	    boolean back) {
	...
	if (node.next.back != null) {
	    int len = getUnweightedPathLenToNamedBranch(node.next.back, name,
		    false);
	...
	if (node.next.next.back != null) {
	    int len = getUnweightedPathLenToNamedBranch(node.next.next.back,
		    name, false);
