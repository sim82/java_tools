/**
 * DISCLAIMER:
 * I (Simon) did not write this code (sicherlich nicht!) an do not claim any copyrights.
 * It is available (without any author and license information) at
 * http://www.cs.utexas.edu/~phylo/datasets/tol/BigDataMatrix.java,
 * so I'm assuming that it is in the public domain.
 * It is just part of this code repository so that it gets compiled
 * automatically and I find it again.
 * Oh yeah, and I did some changes to make it work...
 **/
...
public class BigDataMatrix {
	...
	for (int i = 0; i < Data.length; i++)
	    System.out.println(Name[i] + ": " + "Length= " + Data[i].length()
		    + " Data= " + Data[i]);
	System.out.println();
    }
	...
	try {
		...
		// System.out.println ("hello " + i + "|" + Name[i] + "|" +
		// Data[i] + "|");
	    }
	...
	if (A.length() != B.length()) {
	    System.err
		    .println("Data file contains sequences of unequal length!");
	    return -1;
	}
    ...
    public static BigInteger[] pairwiseSPscore(char[] A1, char[] A2, char[] B1,
	    char[] B2) {
	int i1 = 0, i2 = 0;
	...
	for (i = 0; i < numTaxa; i++) {
		...
		if (i1 == -1)
		    System.out.println(Name[i]
			    + " not found in estimated alignment file.");
	...
	BigDecimal result = new BigDecimal(length).subtract(new BigDecimal(
		score));

	result = result.divide(new BigDecimal(length), DIVISION_SCALE,
		RoundingMode.FLOOR);
    ...
    public static boolean verifyRawSequencesFromAlignments(BigDataMatrix d1,
	    BigDataMatrix d2) {
	if (d1.numTaxa != d2.numTaxa) {
	    throw new RuntimeException("d1.numTaxa != d2.numTaxa. bailing out.");
	...
	for (int i = 0; i < d1.numTaxa; i++) {
	    ...
	    if (jInteger == null) {
		throw new RuntimeException("jInteger == null. bailing out.");
		// return (false);
	    }
		...
		throw new RuntimeException("!raw1.equals(raw2). bailing out.");
		// return (false);
	    }
	}
	...
	/*
	 * String a1 = "ACATA--"; String a2 = "A--TACC"; String b1 = "AC-ATA--";
	 * String b2 = "A-TA--CC";
	 * 
	 * pairwiseSPscore(a1.toCharArray(), a2.toCharArray(), b1.toCharArray(),
	 * b2.toCharArray());
	 */
	...
	if (computeSPScore) {
		...
		System.err
			.println("ERROR: could not verify that raw sequences match from the two input alignments! Aborting.\n");
		System.exit(1);
	    }
	    DM2.SPscore(DM1);
	}
