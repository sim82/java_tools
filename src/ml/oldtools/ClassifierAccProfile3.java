...
public class ClassifierAccProfile3 {
	...
	int[] coverage = null;
	int[] cdist = null;
	boolean lenVsDist = args.length > 0
		&& args[args.length - 1].equals("--sp");
	boolean lenVsDistBM = args.length > 0
		&& args[args.length - 1].equals("--bm");
	...
	for (int i = 0; i < ids.length; i++) {
		...
		id = ids[i];
		ma = MultipleAlignment.loadPhylip(new File(aliTemp.replaceAll(
			"\\%id\\%", id)));
	    }
