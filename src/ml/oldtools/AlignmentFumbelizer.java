...
public class AlignmentFumbelizer {
	...
	for (int i = 1; i < args.length; i++) {
	    MultipleAlignment a = MultipleAlignment
		    .loadFasta(new File(args[i]));
		...
		for (int rc = 0; rc < refseq.length(); rc++) {
		    ...
		    while ((cchar = Character.toUpperCase(cmpseq.charAt(rc
			    + ngaps))) != rchar) {
		...
		// args[i], refseq.length(), cmpseq.length(), cmpseq );
	...
	// MultipleAlignment oa = new MultipleAlignment(ra.seqLen, newnames,
	// newseqs );
