...
public class ClassifierAccProfile {
	...
	final ClassifierLTree.TaxonSMap smap = new ClassifierLTree.TaxonSMap(
		smapFile);
	...
	int N_THREADS = 1;
	ThreadPoolExecutor tpe = new ThreadPoolExecutor(N_THREADS, N_THREADS,
		0L, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(
			N_THREADS * 3));
		...
		public void run() {
			...
			if (atIdx > 0) {
			    ...
			    throw new RuntimeException(
				    "taxon name is expected to contain '@': "
					    + names[i]);
			}
		    ...
		    PrintStream ps;
		    try {
			ps = new PrintStream(new File("prof."
				+ classFile.getName() + ".txt"));
