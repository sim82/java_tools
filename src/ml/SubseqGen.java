...
public class SubseqGen {
	...
	try {
		...
		rns = new PrintStream(new FileOutputStream(alignFile.getName()
			+ "_real_neighbors.txt"));
	    }

	    if (true) {
		sms = new PrintStream(new FileOutputStream(alignFile.getName()
			+ "_smap.txt"));
	    }
	...
	for (int i = starti;; i++) {
		...
		rns.printf("%s\t%s\t%s\t%s\n", FindMinSupport.padchar("" + i,
			0, 4), res.taxon,
			res.taxonNeighbor != null ? res.taxonNeighbor
				: "*NONE*", FindMinSupport
	    ...
	    if (true) {
		try {
		    final File outfile = new File(alignFile.getName() + "_"
			    + FindMinSupport.padchar("" + i, 0, 4) + ".tree");
		...
		String alignName = alignFile.getName() + "_"
			+ FindMinSupport.padchar("" + i, 0, 4);
    ...
    private static void randomSubseqs(String taxon, MultipleAlignment ma,
	    int n, int seqLen) {
	...
	if (n > m) {
	    throw new RuntimeException("trying to draw " + n + " from " + m
		    + " elements. How?");
	}
	...
	for (final LN node : nodelist) {
		...
		if (num == i) {
		    ...
		    // remove the current node (and the branch toward the tip)
		    // by retwiddling of the other two nodes
