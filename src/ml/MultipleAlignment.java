...
public class MultipleAlignment implements Serializable {
	...
	try {
		...
		if (line == null) {
		    throw new RuntimeException("cannot read next line in "
			    + "input file");
		}
	    ...
	    Logger.getLogger(MultipleAlignment.class.getName()).log(
    ...
    // throw new RuntimeException( "error: could not parse taxon name in line "
    // + clines + "(ptr: " + ptr + ", " + line.length() + ")");
    // }
    //
    ...
    // "more different taxon names in data section than in header");
    // }
    //
    ...
    // if( ctaxon != nTaxon ) {
    // throw new RuntimeException(
    // "less different taxon names in data section than in header");
    // }
    //
    ...
    //
    // } catch (IOException ex) {
    // Logger.getLogger(MultipleAlignment.class.getName()).log(Level.SEVERE,
    // null, ex);
    // throw new RuntimeException("bailing out");
    // }
    //
	...
	if (data.length() == 0) {
	    throw new RuntimeException(
		    "sanity check: readLine called on empty line");
	}
	...
	try {
		...
		w = new PrintStream(new GZIPOutputStream(new FileOutputStream(
			file)));
	    } else {
	...
	for (int i = 0; i < nTaxon; i++) {
	    if (names[i] == null) {
		throw new RuntimeException(
			"cannot build name map: name is null: " + i);
	    }
	...
	if (false) {
	    // MultipleAlignment ma = MultipleAlignment.loadPhylipShitty(new
	    // File( "/space/raxml/VINCENT/DATA/500"));
	    long time1 = System.currentTimeMillis();
	    MultipleAlignment mar = MultipleAlignment.loadPhylip(new File(
		    "/space/raxml/VINCENT/DATA/150"));

	    MultipleAlignment ma = MultipleAlignment.loadPhylipShitty(new File(
		    "/space/raxml/VINCENT/DATA/150"));
	    System.out.printf("parse done: %d %s\n", System.currentTimeMillis()
		    - time1, compare(ma, mar));

	    // ma.print();
	} else {
	    long time1 = System.currentTimeMillis();
	    MultipleAlignment.loadPhylipShitty(new File(
	...
	for (int i = 0; i < ma.data.length; i++) {
	    ...
	    if (!ma.nameMap.get(ma.names[i]).equals(
		    mar.nameMap.get(mar.names[i]))) {
		System.out.printf("6\n");
		return false;
	    }
	}
	...
	MultipleAlignment ma = new MultipleAlignment(nTaxon, seqLen, nTaxon
		+ ntxinc);
	...
	try {
		...
		if (len != -1 && len != seq.length()) {
		    throw new RuntimeException(
			    "sequences in fasta file have different lengths. cannot construct MultipleAlignment");
		}

		len = seq.length();
	    }
	    MultipleAlignment ma = new MultipleAlignment(len,
		    names.toArray(new String[names.size()]),
		    seqs.toArray(new String[seqs.size()]));
	...
	for (int i = s; i < e; i++) {
	    ...
	    w.printf("%s%s\n", padSpaceRight(names[oidx], maxNameLen + 2),
		    data[oidx]);
	}
    }
