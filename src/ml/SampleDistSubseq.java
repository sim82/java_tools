...
public class SampleDistSubseq {
	...
	// my interpretation of the (Balzer et al. 2010) "model" of 454 read
	// errors.
	// I think this is correct rather than the N(n, 0.03494 + n * 0.06856)
	// used by grinder,
	// which Balzer et al. only give as an estimation for homopolymers of
	// lengts of 6 and longer.
	...
	public String mutate(String in) {
		...
		if (c != last_char) {
		    ...
		    int hp_len = stretch_len; // this is not really clear: what
					      // is the 'homopolymer length'
					      // in the paper? (what does
					      // length 0 mean?)
		    // double new_len = mean(hp_len) + rnd.nextDouble() *
		    // stddev(hp_len);

		    double new_len = mean(hp_len) + rnd.nextGaussian()
			    * stddev(hp_len);
	...
	for (int i = 0; i < ma.nTaxon; i++) {
		...
		final int len = Math.max(minLen,
			(int) Math.round(gr * sdLen + meanLen));

		outNames[outPtr] = name;
		if (nSamples > 1) {
		    outNames[outPtr] += "_"
			    + FindMinSupport.padchar("" + j, 0, 2);
		}
		outData[outPtr] = FindMinSupport.createRandomSubseqClamped(
			data, len);
	...
	if (!writeFasta) {
	    MultipleAlignment outMa = new MultipleAlignment(ma.seqLen,
		    outNames, outData);
	    outMa.writePhylip(System.out);
	} else {
	    // this is all pretty stupid, but hey, this is java which was
	    // invented for inefficient shit ...
