...
public class ClassifierLTree {
	...
	if (args[0].equals("--auto") || args[0].equals("--autoc")) {
...

		{
		    ...
		    if (!oltFile.canRead()) {
			oltFile = new File(classFile.getParent(),
				"RAxML_originalLabelledTree." + suffix + ".gz");
		    }
		...
		PrintStream out = new PrintStream(new BufferedOutputStream(
			new FileOutputStream(new File(outDir,
				classFile.getName()))));
		try {
	...
	try {
		...
		try {
		    ...
		    // this is really getting ugly: misuse the strange
		    // 'seqSubIdx' stuff to allow for
		    // the removal of arbitrary suffixes connected by an
		    // underscore ...
		    // ... I'm using using ugly code here to honor the
		    // conceptual ugliness us this feature.
		    final int usIdx;
