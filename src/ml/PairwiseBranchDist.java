...
//		System.out.printf( "brnaches with zero length: %d\n", numNull );
	...
	private static double pwdBrute(LN tree, LN[][] sb) {
		...
		for( int i = 0; i < sb.length; i++ ) {
			for( int j = 0; j < i; j++ ) {
				nd[0] = 0;
				bdsum += ClassifierLTree.getPathLenBranchToBranch(sb[i], sb[j], 0.5, nd );
				ndsum += nd[0];
	...
	private static double pwdNew( LN tree, LN[][] sb ) {
		...
		for( int i = 0; i < qs.length; i++ ) {
			...
			// check for early hits == branches directly next top each other
			for( int j = 0; j < 1; j++ ) {
				int oser = sb[i][j].data.localSerial;
				
				for( int k = 0; k < qs.length; k++ ) {
					if( k != i && /*!bbbs.get(bbi.i(i,k))*/ !bbbs[bbi.i(i,k)] ) { 
						if( d[mi.i(k, oser)] > 0 ) {
							if( false ) {
							...
							sum += d[mi.i(k, oser)] + d[mi.i(i, oser)];
							//bbbs.set(bbi.i(i,k));
							//bbbs.set(bbi.i(k,i));
							bbbs[bbi.i(i,k)] = true;
							bbbs[bbi.i(k,i)] = true;
